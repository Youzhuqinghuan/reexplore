...
import tl
from tl import pred_init, train_reg, test_performance, test_performance, predictor
...
parser.add_argument('--core_smiles', type=str, choices=['OC(*)', 'O(*)', 'OCc1ccc(*)cc1'], default='')
...
seed_tl = 1234
batch_size = 64
reaction_dataset = pd.read_csv('/home/hcp/Chem/reaction_yields.csv')
augm = 100
drp_out = 0.0 
sigm_g = 0.0
...
real_comp = pd.read_csv('/home/hcp/Chem/reaction_yields_real.csv')
...
smiles_unbiased, prediction_unbiased, generated_unbiased, molecule_list_unbiased, sc_unbiased = rl_adjustment(rl_agent, tl, args.core_smiles, n_to_generate, sc_path_ub, output_path_ub)
...
import rl
from rl import Reinforcement_random, Reinforcement_topp, Reinforcement_topk
...
if args.trajectories_method == 'random':
    RL_max = Reinforcement_random(rl_agent, tl, Reaction_a_get_reward, get_pred_val)
if args.trajectories_method =='topp':
    RL_max = Reinforcement_topp(rl_agent, tl, Reaction_a_get_reward, get_pred_val)
if args.trajectories_method =='topk':
    RL_max = Reinforcement_topk(rl_agent, tl, Reaction_a_get_reward, get_pred_val)
...
smiles_biased, prediction_biased, generated_biased, molecule_list_biased, sc_biased =rl_adjustment(RL_max.generator, tl,args.core_smiles, n_to_generate, sc_path_bias, output_path_bias)
